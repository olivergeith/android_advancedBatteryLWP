...
public class SimpleCircleV3 extends AdvancedBitmapDrawer {
	...
	private void drawAll(final int level) {
		...
		for (int i = 0; i < 4; i++) {
			...
			final float dicke = maxRadius * 0.15f;
			final float spacing = maxRadius * 0.01f;
			final float ra = aussen - i * dicke;
			final float ri = ra - dicke + spacing;
			final float sw = startWinkel + i * 90;
			// Level
			new LevelPart(center, ra, ri, level, sw, sweep, EZColoring.LevelColors)//
			...
			if (Settings.isShowZeiger()) {
				// Zeiger
				new LevelZeigerPart(center, level, ra, ri, strokeWidth, sw, sweep, EZMode.Einer)//
