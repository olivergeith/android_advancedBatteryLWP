	...
	class MyWallpaperEngine extends Engine implements OnSharedPreferenceChangeListener {
		...
		private void drawBackgroundImage(final Canvas canvas) {
			...
			if (backgroundImage == null // noch nie ein bild geladen?
					|| forcedraw // m�ssen wir neu zeichenen?
					|| customBackgroundChanged() // hat sich das Image ge�ndert?
					|| cWidth != oldWidth // Hat sich die Canvas Gr��e ge�ndert
					|| cHeight != oldHeight) {
				// dann m�ssen wir das backgroundimage neu erstellen
				...
				// wir merken uns mal die Canvasgr��en um sp�ter eine Anderung erkennen
				// zu k�nnen
				oldWidth = cWidth;
				oldHeight = cHeight;
			}
		...
		private Bitmap loadBackgroundImage() {
			...
			// D�rfen wir �berhaupt ins Filesystem?
			...
			// if it is null...wenn das angegebene Bild nicht existiert, oder es nicht
			...
			// erstmal setzen wir die Ziel Gr��e auf die Canvas Gr��e
			int zielH = cHeight;
			int zielW = cWidth;
			// ..und berechnen daraufhin die Breite des Bitmaps (damit
			if (Settings.isKeepAspectRatio()) {
				// anhand der ZielH�he berechnen wir die zielBreite des Bildes, wenn das
				// AspectRatio erhalten bleiben
				// soll
				...
				if (zielW < Math.round(cWidth * 1.4f)) {
					 ...
					 * ...daf�r wird es leider zu hoch :-( also wird sp�ter unten was abgeschnitten <br>
			...
			// Jetzt, wo wir die ZielGr��e haben, skalieren wir das Bild, dass optimal
			// in das Canvas passt!
